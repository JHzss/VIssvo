...
public:
    ...
    static std::string getIMAGE_TOPIC(){return getInstance().IMAGE_TOPIC;}

    static std::string getIMU_TOPIC(){return getInstance().IMU_TOPIC;}
...
//        image_border = (int)fs["FastDetector.image_border"];
        ...
        //! image_topic
        if(!fs["IMAGE_TOPIC"].empty())
            fs["IMAGE_TOPIC"] >> IMAGE_TOPIC;
        //! imu_topic
        if(!fs["IMU_TOPIC"].empty())
