...
namespace ssvo {
    ...
    Matrix3d IMUData::_gyrMeasCov =
            Matrix3d::Identity() * 1.7e-4 * 1.7e-4 / hz_t /** 100*/;       // sigma_g * sigma_g / dt, ~6e-6*10
    Matrix3d IMUData::_accMeasCov =
            Matrix3d::Identity() * 2.0e-3 * 2.0e-3 / hz_t * 100;       // sigma_a * sigma_a / dt, ~8e-4*10
